...
import { FileUploadOptions, UploadedFile, FileValidationResult, FileCompressionOptions } from '../lib/types/file';
...
export function useFileUpload(options: FileUploadOptions = {}) {
  ...
  const validateFile = useCallback((file: File): FileValidationResult => {
    ...
    if (file.size > mergedOptions.maxSize!) {
      errors.push(`File size must be less than ${(mergedOptions.maxSize! / 1024 / 1024).toFixed(1)}MB`);
    }
  ...
  const addFiles = useCallback(async (newFiles: File[]) => {
    ...
    for (const file of newFiles) {
      ...
      const uploadedFile: UploadedFile = {
        ...
        thumbnail,
        compressedSize: processedFile.size !== originalSize ? processedFile.size : undefined,
      };
  ...
  const updateFileStatus = useCallback((fileId: string, status: UploadedFile['uploadStatus'], error?: string) => {
    setFiles((prev) =>
      prev.map((file) =>
        file.id === fileId
